...
Primitive_func::Primitive_func( const char *name, std::function<Variable()> func ) : name( name ), func( func ) {
...
REG_PRIMITIVE_TYPE( reassign ) {
    ...
    if ( va.error() == false && vb.error() == false )
        vm->add_error( "don't known how to reassign {} and {} types", *va.type, *vb.type );
    return args[ 0 ];
}
...
REG_PRIMITIVE_TYPE( sizeof_ptr_in_bits ) {
    return make_Cst_SI32( vm->sizeof_ptr );
}

REG_PRIMITIVE_TYPE( aligof_ptr_in_bits ) {
    return make_Cst_SI32( vm->aligof_ptr );
}

REG_PRIMITIVE_TYPE( is_little_endian ) {
    return make_Cst_Bool( vm->little_endian() );
}

//REG_PRIMITIVE_TYPE( read_sync ) {
//    Variable a = args[ 0 ].ugs( scope );
//    RcString r = *reinterpret_cast<String *>( a.ptr() );
//    File_reader fr( r );
//    if ( fr )
...
//    RcString f = *reinterpret_cast<String *>( vf.ptr() );
//    RcString c = *reinterpret_cast<String *>( vc.ptr() );
//    try {
...
//    RcString name = *reinterpret_cast<String *>( name_var.ptr() );
...
REG_PRIMITIVE_TYPE( constified ) {
    if ( args.size() != 1 )
        return vm->add_error( "__primitive_pointer_on expects exactly 1 argument" );
    Variable a = args[ 0 ];
...
REG_PRIMITIVE_TYPE( ct_display_graph ) {
    vm->display_graph( ".prim" );
    return vm->ref_void;
}

REG_PRIMITIVE_TYPE( rand ) {
    return make_Rand( vm->type_SI32, 32 );
}
...
//    TCI tci;
//    RcString res = args[ 1 ].ugs( scope ).valid_constraint_for( scope, args[ 0 ].ugs( scope ), tci );
...
//    std::string content  = RcString( *s_content  );
//    std::string filename = RcString( *s_filename );
...
//            for( const RcString &name : s->__arguments_names ) {
