...
class Codegen {
public:
    struct ReprType  { void write_to_stream( std::ostream &os ) const { cg->write_repr( os, type               ); } Codegen *cg; TypeInSane  *type;                       };
    ...
    ReprType         repr                          ( TypeInSane *type );
    ReprValue        repr                          ( const Value &val, int prio = PRIO_paren, int flags = 0 );

    virtual void     write_block                   ( StreamSep &os, const Vec<Inst *> &out ) = 0;
    virtual void     write_repr                    ( std::ostream &os, TypeInSane *type ) = 0;
    virtual void     write_repr                    ( std::ostream &os, const Value &value, int prio, int flags ) = 0;

    virtual String   write_func_write_fd           ( TypeInSane *type ) = 0;

    virtual Reg     *new_reg_for                   ( Inst *inst, TypeInSane *type, int nout ) = 0;
