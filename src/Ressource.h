...
public:
    ...
    CanoVal         cano_val       ( const KcSI64 &offset, const KcSI64 &length, TypeInSane *type ) const { return inst->cano_val( nout, offset, length, type ); }
    CanoVal         cano_val       () const { return inst->cano_val( nout ); }
    void           *rcast          ( const KcSI64 &offset );
    TypeInSane     *type           () const { return inst->out_type( nout ); }
